...
lbs () {
  ...
  aws elbv2 create-target-group --name ${vault_load_balancer_name} --region ${region} --protocol TCP --port ${vault_port} --vpc-id ${vpc_id} --target-type instance > ${vault_target_group_file}
  vault_target_group_arn="$(cat ${vault_target_group_file} | jq --raw-output .TargetGroups[0].TargetGroupArn)"
  ...
  bosh interpolate \
    -v env-id="${env_id}" \
    -v target-group="${vault_load_balancer_name}" \
    -v job="vault"  etc/aws/add-target-group.yml > ${state_dir}/cloud-config/add-vault-tg.yml
}
